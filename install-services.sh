...
runtime="$HOME/.zombie"
[[ -x "$runtime/bin/zombied" ]] || { echo 'Run install-termux.sh first.' >&2; exit 1; }
[[ $# -gt 0 ]] || { echo 'Usage: install-services.sh spotify|threadfin [...]' >&2; exit 1; }
for tool in go clang python3 git tar; do command -v "$tool" >/dev/null; done
umask 077
for name in "$@"; do
  case "$name" in
    spotify)
      source_name=go-librespot; commit=57d7278d94a9233060c2a6238f5926ffd1e72de4
      command -v pkg-config >/dev/null
      pkg-config --exists ogg vorbis flac libmpg123 || { echo 'Install native libogg, libvorbis, libflac, mpg123 and pkg-config development files.' >&2; exit 1; }
      ;;
    threadfin) source_name=threadfin; commit=6b9c0ccf16164eb362af0a44660228267734c5aa;;
    *) echo "Unsupported native package: $name" >&2; exit 1;;
  esac
  upstream="$repo/third_party/sources/$source_name"
  [[ $(git -C "$upstream" rev-parse HEAD) == "$commit" ]] || { echo 'Run make references first.' >&2; exit 1; }
...
PYTHON
  ...
  service="$PREFIX/var/service/zombie-$name";mkdir -p "$service";touch "$service/down"
  if [[ $name == spotify ]]; then
    cat > "$service/run" <<'RUN'
