...
if $with_cast; then
  ...
  [[ $(git -C "$upstream" rev-parse HEAD) == 048255986f7e04b859b4c4efe651448ec785ecd4 ]] || { echo 'Restore pinned sources with make references first.' >&2; exit 1; }
  command -v tar >/dev/null
fi
if $with_youtube; then
  command -v node >/dev/null && command -v npm >/dev/null || { echo 'Install a native Node >=22 runtime and npm.' >&2; exit 1; }
fi
umask 077
runtime="$HOME/.zombie"
...
if $with_cast; then
  ...
  service="$PREFIX/var/service/zombie-mediamtx";mkdir -p "$service";touch "$service/down"
  cat > "$service/run" <<'RUN'
...
NODE
  service="$PREFIX/var/service/zombie-youtube";mkdir -p "$service";touch "$service/down"
  cat > "$service/run" <<'RUN'
#!/data/data/com.termux/files/usr/bin/sh
exec 2>&1
...
service="$PREFIX/var/service/zombied";mkdir -p "$service";touch "$service/down"
cat > "$service/run" <<'RUN'
#!/data/data/com.termux/files/usr/bin/sh
exec 2>&1
set -a
...
RUN
chmod +x "$service/run"
{ go version; ffmpeg -version | sed -n '1p'; ffprobe -version | sed -n '1p'; } > "$runtime/build-info.txt"
